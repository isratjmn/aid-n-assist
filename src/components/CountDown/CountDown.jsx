...
const CountDown = () => {
	...
	const TotalStorePayments = (
		storePaymentCount.reduce((sum, item) => sum + item.price, 0) / 100
	).toFixed(2);
	const TotalStripePayments = (
		stripeCount.reduce((sum, item) => sum + item.price, 0) / 100
	).toFixed(2);
	const TotalDonationPayments = (
		donation.reduce((sum, item) => sum + item.donationAmount, 0) / 100
	).toFixed(2);

	return (
		<ScrollTrigger
			onEnter={() => setOnTheCounter(true)}
			onExit={() => setOnTheCounter(false)}
		>
			<div
				className="mt-20 counter_section"
				data-aos="fade-up"
						...
						<h2>
							...
							<p className="mt-3 text-4xl text-blue-600">
								...
								{onTheCounter && (
									<CountUp
										start={0}
										end={TotalStripePayments}
										duration={2}
										delay={0}
									></CountUp>
								)}
								<sup className="text-white text-3xl">+</sup>
							</p>
						</h2>
					</div>
